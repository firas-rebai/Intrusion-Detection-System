...
attributes = ['duration', 'protocol_type', 'service', 'flag', 'src_bytes', 'dst_bytes', 'land', 'wrong_fragment',
...
def process():
    ...
    values_list = [request.form[attr] for attr in attributes]
    columns = (
    ...
    predictions = [forest.predict(data.iloc[:, :12]), knn.predict(data.iloc[:, :12]),
                   adaboost.predict(data.iloc[:, :12])]
    results = []
